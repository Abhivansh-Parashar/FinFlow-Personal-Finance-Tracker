...
export default function StatCard({
                                     ...
                                     icon: Icon, accentColor = 'green',
                                     prefix = '₹', large = false, subtitle
                                 }) {
    ...
    return (
        <div className="stat-card card fade-in"
             style={{ '--accent-bg': colors.bg, '--accent-border': colors.border, '--accent-text': colors.text }}>
            ...
            {/* FIX: subtitle prop for secondary info (e.g. "of ₹50k budgeted") */}
            {subtitle && (
                <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: 4 }}>{subtitle}</div>
            )}
        ...
        .stat-card::before {
          ...
          width: 100px; height: 100px; background: var(--accent-bg);
          border-radius: 50%; transform: translate(30%, -30%); pointer-events: none;
        }
        .stat-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
        .stat-title { font-size: 0.78rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.07em; font-weight: 600; }
        .stat-icon-wrap { width: 32px; height: 32px; background: var(--accent-bg); border: 1px solid var(--accent-border); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: var(--accent-text); }
        .stat-value { font-family: var(--font-display); font-weight: 700; letter-spacing: -0.03em; margin-bottom: 10px; line-height: 1; }
        .stat-change { display: flex; align-items: center; gap: 4px; font-size: 0.78rem; font-weight: 500; }
        .change-label { color: var(--text-muted); font-weight: 400; }
      `}</style>
        </div>
    )
}
